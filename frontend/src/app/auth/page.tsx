...
export default function AuthPage() {
  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        login({ id: realUser.id, email: realUser.email, username: realUser.email.split("@")[0], role: realUser.role, status: realUser.status }, data.access_token);
  ...
  const handleAuth = async (type: "login" | "register") => {
    ...
    try {
      if (type === "register") {
        await api.post("/v1/auth/register", { email, password, role });
        if (role === "mentor") {
          toast.success("Mentor account created. Waiting for admin approval.");
        } else {
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-4 mt-2">
                        <label className="flex items-center gap-2 cursor-pointer">
                          <input type="radio" name="role" value="student" checked={role === "student"} onChange={() => setRole("student")} className="accent-[#5B5FEF]" />
                          <span className="text-sm font-medium">Student</span>
                        </label>
                        <label className="flex items-center gap-2 cursor-pointer">
                          <input type="radio" name="role" value="mentor" checked={role === "mentor"} onChange={() => setRole("mentor")} className="accent-[#5B5FEF]" />
